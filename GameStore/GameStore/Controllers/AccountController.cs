...
namespace GameStore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Addresses(string userId)
        {
            var model = new List<AddressViewModel>();
            var addresses = string.IsNullOrEmpty(userId) ? 
                AccountManager.Addresses.ToList() : 
                AccountManager.GetUserAddresses(userId);
            foreach (var address in addresses)
            {
                model.Add(address.ToAddressViewModel());
            }
            return View(model);
        }
        ...
        public ActionResult AddAddress()
        {
            ...
            ViewBag.Regions = GeneralHelper.SelectListForEnum(typeof(Region));
            return View(model);
        }
        ...
        public ActionResult DeleteAddress(Guid? id)
        {
            if (id.HasValue)
            {
                AccountManager.DeleteAddress(id.Value);
                AccountManager.Save();
            }
            return RedirectToAction("Addresses", AccountHelper.GetLoggedUserId());
        }
